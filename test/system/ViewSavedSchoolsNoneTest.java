...
public class ViewSavedSchoolsNoneTest {
	...
	public void setUp() throws Exception {
		...
		UniversityController.saveSchool(testUser.getUsername(), uni.getSchool());
		
		UserInteraction.login(testUser.getUsername(), testUser.getPassword());
	}

	@After
	public void tearDown() throws Exception {
		DatabaseController.removeUser(testUser.getUsername());
		DatabaseController.removeUser(testUser2.getUsername());
		DatabaseController.deleteUniversity(uni.getSchool());
	}

	@Test
	public void testNoSavedSchools() {
		Assert.assertNotNull(UserInteraction.getSavedSchools());
	}

	@Test
	public void testSavedSchools() {
		Assert.assertEquals(1, UserInteraction.getSavedSchools().size());
	}
}
